...
void show(QUE *n, struct Proc pcbTable[]){
  ...
  while (n != NULL){
    ...
    printf("%2d, %3d,  %3d, %8d, %5d, %10d, %3d\n",
	   ...
	   proc.value, proc.t_start, proc.t_total_time_used);
    n = n->next;
  }
  printf("\n");
  return;
...
void show_by_priority(QUE *n, struct Proc pcbTable[], int priority){
  ...
  while (n != NULL){
    if(pcbTable[n->pid].priority == priority){
      proc = pcbTable[n->pid];
      printf("pc, pid, ppid, priority, value, start time, CPU time used so far\n");
      printf("%2d, %3d,  %3d, %8d, %5d, %10d, %3d\n",proc.pc, proc.pid, proc.ppid, proc.priority,proc.value, proc.t_start, proc.t_total_time_used);
      count++;
    }
    n = n->next;
  }
...
void processManagerProcess(int rfd, char *init_program)
{
  ...
  while (fgets(buffer, BUFSIZ, fp) != NULL) {
    ...
    if(!strcmp(buffer, "Q\n") || !strcmp(buffer, "q\n")){
      ...
      cpu.pc++;
      cpu.t_remain--;
      //creamos otra variable que cuente el tiempo total de uso de la CPU
      pcbTable[cpu.pid].t_total_time_used++;
      if(!strcmp(cmd[0], "S")){
